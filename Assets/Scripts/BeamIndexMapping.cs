...
public class BeamIndexMapping : MonoBehaviour
{
    ...
    private void CreateBeamGameObject()
    {
        ...
        // Add NearInteractionTouchable script to make the beam touchable
        NearInteractionTouchable touchable = beam.AddComponent<NearInteractionTouchable>();

        // Attach a collider to the beam
        BoxCollider collider = beam.AddComponent<BoxCollider>();
        collider.size = new Vector3(1f, 1f, 10f);
    ...
    {
        if (Vector3.Distance(eventData.InputData, beam.transform.position) < 0.5f)
        {
      Debug.Log("Beam has been touched!");
            // Add your custom logic here when the beam is touched
        }
    }
